...
const socialLinks = [
  { icon: FacebookIcon, href: "https://www.facebook.com/duke7925" },
  { icon: InstagramIcon, href: "https://www.instagram.com/duke.nd_/" },
  { icon: GithubIcon, href: "https://github.com/dukeWst" },
];
  ...
  return (
    <footer className="relative bg-[#020205] text-gray-400 border-t border-white/5 overflow-hidden isolate">
      ...
      <div className="absolute -bottom-20 -right-20 select-none opacity-[0.02] pointer-events-none">
        <span className="text-[25rem] font-black text-white leading-none tracking-tighter">
          HX
        </span>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8 lg:py-28 relative z-10">
        <div className="xl:grid xl:grid-cols-3 xl:gap-16">
          <div className="space-y-10">
            <div className="flex flex-col gap-2">
              ...
              <span className="text-[10px] font-mono font-bold text-cyan-500/60 uppercase tracking-[0.4em] ml-1">
                Universal Tech Hub
              </span>
            </div>
            <p className="text-sm leading-relaxed text-gray-500 max-w-xs font-medium">
              The ultimate destination for tech enthusiasts, developers, and
            ...
            <div className="flex space-x-5">
              {socialLinks.map((social, idx) => (
                <a
                  key={idx}
                  href={social.href}
                  className="h-10 w-10 flex items-center justify-center rounded-xl bg-white/[0.03] border border-white/5 text-gray-500 hover:text-cyan-400 hover:bg-cyan-500/5 hover:border-cyan-500/20 transition-all duration-300 group"
                >
                  <social.icon
                    size={20}
          ...
          <div className="mt-16 grid grid-cols-2 gap-12 xl:col-span-2 xl:mt-0">
            <div className="md:grid md:grid-cols-2 md:gap-12">
              {footerSections.slice(0, 2).map((section) => (
                <div key={section.title} className={section.title === "Resources" ? "mt-12 md:mt-0" : ""}>
                  <h3 className="text-xs font-black text-white uppercase tracking-[0.2em] mb-8 font-mono">
                    {section.title}
                  </h3>
                  <ul role="list" className="space-y-5">
                    {section.links.map((link) => (
                      <li key={link.name}>
                        <a
                          href={link.href}
                          className="text-sm font-medium hover:text-cyan-400 transition-colors flex items-center gap-2 group"
                        >
                          <div className="w-1 h-1 rounded-full bg-cyan-500/20 group-hover:bg-cyan-400 group-hover:scale-150 transition-all"></div>
                          {link.name}
                        </a>
                      </li>
                    ))}
        ...
        <div className="mt-20 border-t border-white/5 pt-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-xs leading-5 text-gray-600 font-mono font-bold uppercase tracking-widest">
            &copy; 2025 HyperX Universal. All rights reserved.
          </p>
          <div className="flex gap-8">
            <span className="text-[10px] text-cyan-500/40 font-mono uppercase tracking-widest">
              v2.4.0 Stable
            </span>
