...
const ProductList = ({ handleSearchSubmit, search, isLoading, products, formatCurrency, setSearch }) => {
    ...
    return (
        // KEY FIX: h-full flex flex-col overflow-hidden (Khóa khung ngoài)
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <form onSubmit={handleSearchSubmit} className="relative group flex-1 md:w-80">
                            ...
                            <input
                                ...
                                className="block w-full pl-10 pr-3 py-2.5 border border-white/10 rounded-xl leading-5 bg-white/5 text-gray-300 placeholder-gray-500 focus:outline-none focus:bg-white/10 focus:ring-1 focus:ring-indigo-500/50 focus:border-indigo-500/50 sm:text-sm transition-all shadow-lg"
                            />
                        </form>

                        <Link to="/create-product">
                            <button className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2.5 rounded-xl font-semibold transition-all shadow-lg shadow-indigo-500/20 active:scale-95 text-sm whitespace-nowrap">
                                <Plus size={18} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-32">
                        {products.map((item) => (
                            <Link to={`/product/${item.id}`} key={item.id} className="group relative block h-full">
                                <div className="absolute -inset-0.5 bg-gradient-to-b from-indigo-500 to-purple-500 rounded-2xl opacity-0 group-hover:opacity-30 blur transition duration-500"></div>
                                ...
                                <div className="relative h-full bg-[#0B0D14] border border-white/10 rounded-2xl overflow-hidden flex flex-col transition-transform duration-300 group-hover:-translate-y-1">
                                    ...
                                    <div className="relative aspect-video overflow-hidden bg-gray-900">
                                        ...
                                        <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 shadow-xl">
                                            <p className={`font-bold text-xs ${item.price === 0 ? "text-green-400" : "text-white"}`}>
                                    ...
                                    <div className="p-5 flex flex-col flex-1">
                                        <h3 className="text-lg font-bold text-gray-100 group-hover:text-indigo-400 transition-colors line-clamp-1 mb-2">{item.name}</h3>
                                        ...
                                        {item.tag && item.tag.length > 0 && (
                                            <div className="flex items-center gap-2 pt-4 border-t border-white/5 flex-wrap">
                                                {item.tag.slice(0, 3).map((t, index) => (
                                                    <span key={index} className="flex items-center gap-1 text-[10px] uppercase tracking-wider px-2 py-1 rounded-md border font-medium bg-white/5 text-gray-400 border-white/10 group-hover:border-indigo-500/30 group-hover:text-indigo-200 transition-colors">
                                                        {getTagIcon(t)} {t}
                                                    </span>
                                                ))}
                                                {item.tag.length > 3 && <span className="text-[10px] text-gray-500">+{item.tag.length - 3}</span>}
                                            </div>
                                        )}
                                    </div>
                                </div>
