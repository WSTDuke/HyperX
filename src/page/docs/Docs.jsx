                  ...
                  return (
                    <li key={item.id}>
                      ...
                      >
                        <Icon
                          size={16}
                          className={`${activeDoc === item.id ? "text-cyan-400" : "text-gray-600 group-hover:text-gray-400"} transition-colors`}
                        />
                        {item.title}
                      </button>
                    </li>
...
const DocsPage = () => {
  ...
  const handleSearch = (e) => {
    ...
    const searchResults = keys.filter((key) =>
      mockDocsContent[key].title.toLowerCase().includes(query),
    );
  ...
  const displayedContent =
    customContent || mockDocsContent[activeDoc] || mockDocsContent["intro"];
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-12 items-center justify-between">
              ...
              >
                ...
                <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                  <span className="text-gray-600 text-[10px] font-bold border border-white/10 rounded-lg px-2 py-0.5 bg-white/5">
                    SEARCH
                  </span>
                </div>
              </form>
                ...
                <article className="relative border border-white/10 bg-white/[0.02] backdrop-blur-[100px] rounded-[2.5rem] p-8 md:p-14 lg:p-16 shadow-2xl overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-8">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] text-cyan-500/80">
                        HyperX Build v1.0.4
                      </span>
                    </div>
                    ...
                    <div className="doc-content-premium">
                      <ReactMarkdown
                        ...
                        components={{
                          h2: ({ ...props }) => (
                            <h2
                              className="text-xl md:text-2xl font-bold text-white mt-12 mb-6 flex items-center gap-3"
                              {...props}
                            />
                          ),
                          h3: ({ ...props }) => (
                            <h3
                              className="text-lg font-bold text-cyan-400 mt-8 mb-4"
                              {...props}
                            />
                          ),
                          p: ({ ...props }) => (
                            <p
                              className="text-gray-400 text-base md:text-lg leading-relaxed mb-6 opacity-90"
                              {...props}
                            />
                          ),
                          ul: ({ ...props }) => (
                          ...
                          li: ({ ...props }) => (
                            <li className="flex items-start gap-4 group/li">
                              ...
                              <span className="text-gray-400 group-hover/li:text-gray-100 transition-colors text-base md:text-lg italic font-medium">
                                {props.children}
                              </span>
                            </li>
                          ),
                          strong: ({ ...props }) => (
                            <strong
                              className="text-cyan-300 font-bold px-1.5 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/20"
                              {...props}
                            />
                          ),
                          a: ({ ...props }) => (
                            <a
                              className="text-cyan-400 font-bold underline underline-offset-8 decoration-2 decoration-cyan-500/30 hover:decoration-cyan-400 transition-all"
                              {...props}
                            />
                          ),
                          code: ({ inline, ...props }) =>
                            inline ? (
                              <code
                                className="bg-white/5 text-cyan-200 px-2 py-0.5 rounded text-sm font-mono border border-white/10"
                                {...props}
                              />
                            ) : (
                              <code {...props} />
                      ...
                      <div className="mt-24 pt-12 border-t border-white/5">
                        ...
                        >
                          <div className="text-left">
                            <span className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] mb-3">
                              Continue Reading
                            </span>
                            <span className="text-xl md:text-2xl font-extrabold text-white group-hover:text-cyan-400 transition-colors">
                              {mockDocsContent[displayedContent.next]?.title ||
                                "Nest Section"}
                            </span>
                          </div>
                          <div className="w-14 h-14 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-cyan-500 group-hover:shadow-[0_0_30px_rgba(34,211,238,0.5)] transition-all duration-500">
                            <ArrowRight
                              size={28}
                              className="text-cyan-500 group-hover:text-black transition-all group-hover:translate-x-1"
                            />
                          </div>
                        </button>
                      </div>
