...
export const ChatInput = React.memo(
  ({
    ...
    return (
      <div
        className={`w-full transition-all duration-500 ${isCentered ? "" : "backdrop-blur-xl bg-[#05050A]/80 border-t border-white/5 p-4 sm:p-6"}`}
      >
        <div className={`mx-auto ${isCentered ? "max-w-2xl" : "max-w-4xl"}`}>
          {}
          {alert && (
            <div
              className={`mb-4 p-3 text-sm rounded-xl flex items-center justify-between animate-in fade-in slide-in-from-bottom-2 ${alert.type === "error" ? "bg-red-500/10 text-red-300 border border-red-500/20" : "bg-cyan-500/10 text-cyan-300 border border-cyan-500/20"}`}
            >
