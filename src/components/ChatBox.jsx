...
const ChatSession = ({
  ...
  useEffect(() => {
    ...
    const setupConversation = async () => {
      ...
      if (existing) {
        ...
        const myClearedAt =
          myId === existing.user_1
            ? existing.user_1_cleared_at
            : existing.user_2_cleared_at;
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("hyperx-chat-deleted", handleCleared);
  }, [conversation]);
  ...
  const handleClearChat = async () => {
    ...
    const now = new Date().toISOString();
    const field =
      currentUser.id === conversation.user_1
        ? "user_1_cleared_at"
        : "user_2_cleared_at";
    await supabase
      .from("conversations")
  ...
  return (
    <div className="w-[360px] h-[500px] bg-[#0B0D14] border border-white/10 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-in slide-in-from-right-8 duration-300 ring-1 ring-white/5 pointer-events-auto">
      <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between">
        <Menu as="div" className="relative">
          <MenuButton className="flex items-center gap-3 hover:bg-white/5 p-1 -m-1 rounded-2xl transition-all group">
            ...
            <MoreVertical
              size={14}
              className="text-gray-600 group-hover:text-cyan-500 opacity-0 group-hover:opacity-100 transition-all"
            />
          </MenuButton>
              ...
              <MenuItem>
                {({ active }) => (
                  <button
                    ...
                    className={`${active ? "bg-cyan-500/10 text-cyan-400" : "text-gray-400"} group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-bold uppercase tracking-widest transition-all`}
                  >
              ...
              <MenuItem>
                {({ active }) => (
                  <button
                    ...
                    className={`${active ? "bg-red-500/10 text-red-400" : "text-gray-400"} group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-bold uppercase tracking-widest transition-all disabled:opacity-50`}
                  >
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto custom-scrollbar bg-gradient-to-b from-transparent to-black/20"
      >
        <div className="min-h-full flex flex-col justify-end p-4 space-y-1">
          {loading && messages.length === 0 ? (
            ...
            messages.map((msg, i) => {
              ...
              const showSeparator = shouldShowTimestamp(
                msg.created_at,
                prevMsg?.created_at,
              );
              ...
              return (
                <React.Fragment key={msg.id || i}>
                  ...
                  <div
                    className={`flex items-end gap-2 group ${isMe ? "flex-row-reverse" : "flex-row"} animate-in fade-in duration-300`}
                  >
                    {!isMe && (
                      <div className="mb-1">
                        <UserAvatar
                    ...
                    <div
                      className={`max-w-[75%] rounded-[1.2rem] px-4 py-2.5 text-sm shadow-sm break-words whitespace-pre-wrap ${isMe ? "bg-cyan-600 text-white" : "bg-white/5 text-gray-200 border border-white/5"}`}
                    >
                      {msg.content}
                    </div>
      ...
      >
        <div className="flex items-center gap-2 bg-black/40 border border-white/10 rounded-2xl p-1.5 focus-within:border-cyan-500/50 transition-all shadow-inner">
          ...
          <button
            ...
            className="p-2 bg-cyan-600 text-white rounded-xl hover:bg-cyan-500 disabled:opacity-30 transition-all shadow-lg active:scale-90"
          >
            <Send size={18} />
          </button>
        </div>
      ...
      <Transition show={isConfirmModalOpen} as={React.Fragment}>
        ...
        >
          ...
          <div className="fixed inset-0 overflow-y-auto flex min-h-full items-center justify-center p-4">
            ...
            >
              <DialogPanel className="w-full max-w-md transform overflow-hidden rounded-3xl bg-[#0B0D14] border border-white/10 p-6 shadow-2xl ring-1 ring-white/5 transition-all">
              <DialogTitle className="text-lg font-bold text-white uppercase mb-4 text-center">
                Delete Conversation?
              </DialogTitle>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-3 rounded-xl bg-white/5 text-gray-400 text-xs font-bold uppercase"
                >
                  Cancel
                </button>
                <button
                  onClick={handleClearChat}
                  className="flex-1 px-4 py-3 rounded-xl bg-red-600 text-white text-xs font-bold uppercase hover:bg-red-500"
                >
                  Delete All
                </button>
              </div>
      ...
      <div className="flex-shrink-0">
        {activeSessionData && (
          <ChatSession
            ...
            onRemove={() => {
              setActiveSessions((prev) =>
                prev.filter((s) => s.id !== activeSessionData.id),
              );
